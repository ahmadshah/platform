...
class Container {
	...
	/**
	 * Indicate whether to add file mtime information
	 *
	 * @var bool
	 */
	public $asset_mtime = false;
	...
	public function __construct($name)
	{
		...
		$this->asset_mtime = Config::get('application.asset_mtime', false);
	}
	...
	protected function asset($group, $name)
	{
		...
		if (filter_var($asset['source'], FILTER_VALIDATE_URL) === false)
		{
			...
			// extract the file.
			$file = path('public').$asset['source'];

			$asset['source'] = $this->path($asset['source']);

			if ($this->asset_mtime)
			{
				$asset['source'] = $asset['source'].'?'.filemtime($file);
			}
		}
