...
class Lang {
	...
	public static function file($bundle, $language, $file)
	{
		...
		// Look inside application/language/lang/bundles/bundle-name to allow languages to be stored inside
		// the application folder. This allows bundle runtime languages to maintain even after
		// the bundle is upgraded.
		if ($bundle !== DEFAULT_BUNDLE)
		{
			$path = static::path(DEFAULT_BUNDLE, $language, "bundles/{$bundle}/{$file}");

			if (file_exists($path))
			{
				$app_lines = require $path;

				$lines = array_merge($app_lines, $lines);
			}
		}
