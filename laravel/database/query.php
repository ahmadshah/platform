...
class Query {
	...
	/**
	 * Indicates if the query operates on empty result sets.
	 *
	 * This can be set to true by certain constraints in order to save a query.
	 *
	 * @var bool
	 */
	protected $empty_set = false;
	...
	public function where_in($column, $values, $connector = 'AND', $not = false)
	{
		...
		if (empty($values))
		{
			// "NOT IN ()" is the same as no WHERE condition
			if ($not)
			{
				return $this;
			}
			// "IN ()" will cause the result to be empty
			else
			{
				$this->empty_set = true;
			}
	...
	{
		// If this query is not going to fetch any rows, we won't run it.
		if ($this->empty_set)
		{
			return false;
		}
	...
	{
		// If this query is not going to affect any rows, we won't run it.
		if ($this->empty_set)
		{
			return 0;
		}
