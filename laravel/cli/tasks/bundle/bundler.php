...
class Bundler extends Task {
	...
	public function install($bundles)
	{
		foreach ($this->get($bundles) as $bundle)
		{
			...
			{
				Console::error("Bundle {$bundle['name']} is already installed.");
			...
			Console::write_inline("Fetching [{$bundle['name']}]...");

			$this->download($bundle, $path);

			Console::write_inline("done!");
			Console::enter();
			Console::write("Bundle installed.", "green");
		}
	}
	...
	public function upgrade($bundles)
	{
		...
		foreach ($bundles as $name)
		{
			if ( ! Bundle::exists($name))
			{
				Console::error("Bundle [{$name}] is not installed!");
			...
			Console::write("Bundle [{$name}] has been upgraded!", "green");
		}
	}
	...
	protected function get($bundles)
	{
		...
		foreach ($bundles as $bundle)
		{
			...
			if ($response['status'] == 'not-found')
			{
				throw new InvalidArgumentException("There is no bundle named [$bundle].");
			}
	...
	protected function retrieve($bundle)
	{
		...
		if ( ! $response)
		{
			throw new RuntimeException("The bundle API is not responding.");
		}
