...
#### Generating a link with HTML content

By default, HTML::link would convert HTML character to entities for security purposes. However if you wish to use HTML attribute, wrap it inside `HTML::raw()` which would tell Laravel to skip the process

	echo HTML::link('user/profile', HTML::raw('<i class="icon-user"></i> User Profile')); 
